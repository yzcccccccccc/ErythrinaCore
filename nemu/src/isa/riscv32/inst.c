...
#define immB() do { *imm = (SEXT(BITS(i, 31, 31), 1) << 11) | (BITS(i, 30, 25) << 4) | (BITS(i, 11, 8) << 1) | (BITS(i, 7, 7) << 10);} while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, R(rd) = ((int32_t)src1 < (int32_t)imm));    // signed!
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = (src1 < imm));                      // unsigned
  ...
  // B-Type
  int32_t s_src1 = src1, s_src2 = src2;
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, {if (s_src1 == s_src2) s->dnpc += imm;});
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, {if (s_src1 != s_src2) s->dnpc += imm;});
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, {if (s_src1 < s_src2) s->dnpc += imm;});
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, {if (s_src1 >= s_src2) s->dnpc += imm;});
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, {if (src1 < src2) s->dnpc += imm;});
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, {if (src1 >= src2) s->dnpc += imm;});
