...
if __package__ or "." in __name__:
	from .libUT_DivDebug import *
else:
	from libUT_DivDebug import *


class DUTDivDebug(DutUnifiedBase):
	...
	def __init__(self, waveform_filename=None, coverage_filename=None, *a, **kw):
		...
		self.io_res_valid = xsp.XPin(xsp.XData(0, xsp.XData.Out), self.event)
		...
		self.io_res_valid.BindDPIRW(DPIRio_res_valid, DPIWio_res_valid)
		...
		self.port.Add("io_res_valid", self.io_res_valid.xdata)
...
if __name__=="__main__":
	dut=DUTDivDebug("libDPIDivDebug.so")
	dut.Step(1)
