...
class IDU2BPU extends Bundle with BPUtrait{
...
class EXU2BPU extends Bundle with BPUtrait{
...
class BPUIO extends Bundle with BPUtrait{
    val ID2BPU      = new IDU2BPU
    val EX2BPU      = new EXU2BPU
...
class BPU extends Module with BPUtrait{
    val io      = IO(new BPUIO)
    val tar_pc  = io.ID2BPU.src1 + io.ID2BPU.src2
    val dnpc    = Mux(io.ID2BPU.bpuop === BPUop.jalr, Cat(tar_pc(XLEN - 1, 1), 0.B), tar_pc);

    val redirect = LookupTree(io.ID2BPU.bpuop, List(
        BPUop.nop   -> 0.B,
